...
/**
 * Servlet used to display all audiobooks data
 *
 * @author Kamil Musiałowski
 * @version 1.5
 */
@WebServlet("/ShowAllAudiobooksServlet")
public class ShowAllAudiobooksServlet extends HttpServlet {

    /**
     * Processes requests for both HTTP <code>GET</code> and <code>POST</code>
     * methods. Displays all audiobooks info.
     *
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        out.println("<html>\n<body>\n"
                ...
                + "</form>\n"
                + htmlTableGenerator(modelToTable())
                + "</body>\n</html>");
    }
    ...
    private String htmlTableGenerator(String[][] data) {
        ...
        for (String[] data1 : data) {
            ...
            generatedTable
                    ...
                    + "<th>" + data1[2] + "</th>\n" + "<th>" + data1[3] + "</th>\n"
                    + "<th>" + data1[4] + "</th>\n"
                    + "<th> <form action=\"LendAudiobookServlet\"> <input type=\"submit\" name=\"" + data1[0] + "\" value=\"Lend\"/>"
    ...
    /**
     * Prepares String array containing data from DB
     *
     * @return Strng array containig data from DB
     */
    private String[][] modelToTable() {
        int size = 0;
        Connection connection = (Connection) this.getServletContext().getAttribute("connection");
        String[][] data = new String[1][1];
        try {
            ResultSet rs = ((Model)this.getServletContext().getAttribute("model")).selectAudiobooksAll();
            if (rs != null) {
                rs.last();
                size = rs.getRow();
                rs.beforeFirst();
